...
#-------------------------Loading Experimental data as res---------
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
def Loadexp(exp,scale):
    exp=exp[0:70,:]
    # for i in range(0,70):
...
def Training(epochs,model_name,dataloader,PATH,res):
...
if  __name__=='__main__':
    ...
    model_name='Yb2O3.pt' #only contains the material name and .pt
    PATH = r"R:\SpinwAI\Yb2O3\model_"+model_name
    ...
    model_name='Yb2O3.pt' #change model name and path if we want to test different models
    PATH = r"R:\SpinwAI\Yb2O3\model_"+model_name
    mat_test=spio.loadmat(r"R:\SpinwAI\Yb2O3\Matscale\matscale01.mat") #01-19 sets of data have been seen. 20th data set is not seen
    J_test=spio.loadmat(r"R:\SpinwAI\Yb2O3\Jscale\Jscale01.mat") #01-19 sets of data have been seen. 20th data set is not seen
    count=2
    Testing(model_name,mat_test,J_test,count)

    #testing on expermental data
    test_exp(exp,model)